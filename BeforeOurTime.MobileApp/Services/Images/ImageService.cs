...
namespace BeforeOurTime.MobileApp.Services.Items
{
    ...
    public class ImageService : IImageService
    {
        ...
        /// <summary>
        /// Read multiple images based on a list of unique image identifiers
        /// </summary>
        /// <param name="imageIds">List of unique image identifiers</param>
        /// <returns></returns>
        public async Task<List<Image>> ReadAsync(List<Guid> imageIds = null)
        {
            await Task.Delay(0);
            var images = Images.Where(x => imageIds.Contains(x.Id)).ToList();
            return images;
            //var response = await MessageService.SendRequestAsync<CoreReadImageResponse>(new CoreReadImageRequest()
            //{
            //    ImageIds = imageIds
            //});
            //return response.CoreReadImageEvent.Images;
        }
        /// <summary>
